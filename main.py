...
async def fetch_transactions(session: ClientSession, address: str) -> List[dict]:
    ...
    Returns:
        List[dict]: List of transaction dictionaries, or an empty list on failure.
    """
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            logger.warning(f'Attempt {attempt}/{MAX_RETRIES}: Network error while fetching transactions for {address}: {e}')
...
async def process_transactions(session: ClientSession, transactions: List[dict], processed_addresses: Set[str], depth: int) -> List[Tuple[str, str, float]]:
    ...
    for tx in transactions:
        ...
        # Add tasks to process addresses recursively
